...
class Total extends \Simi\Simiconnector\Helper\Total
{
    ...
    public function showTotalOrder($order)
    {
        ...
        // add giftcard codes discount to order detail
        $data['aw_giftcard_amount'] = $order->getData('aw_giftcard_amount');
        $data['base_aw_giftcard_amount'] = $order->getData('base_aw_giftcard_amount');
        if ($order->getExtensionAttributes() && $order->getExtensionAttributes()->getAwGiftcardCodes()) {
            ...
            foreach ($giftcards as $giftcard) {
                $awGcCodes[] = $giftcard->getGiftcardCode();
            }
            $data['aw_giftcard_codes'] = implode(', ', $awGcCodes);
        }
        return $data;
    }
