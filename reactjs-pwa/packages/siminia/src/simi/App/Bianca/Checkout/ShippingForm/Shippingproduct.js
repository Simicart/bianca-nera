...
const Shippingproduct = props => {
    ...
    if (cart && cart.totals && cart.totals.items && designer) {
        return cart.totals.items.map((totalItem, index) => {
            let cartDetailsItemToMerge = {}

            if (cart.details && cart.details.items) {
                cart.details.items.every(cartDetailItem => {
                    if (cartDetailItem.item_id === totalItem.item_id) {
                        cartDetailsItemToMerge = cartDetailItem
                        return false
                    }
                    return true
                }
            ...
            const item = { ...totalItem, ...cartDetailsItemToMerge }
            if (
