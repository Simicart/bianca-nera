...
namespace Simi\Simicustomize\Override\Helper\Checkout;

class Payment extends \Simi\Simiconnector\Helper\Checkout
{
    ...
    public function getDetailsPayment($method)
    {
        ...
        switch ($type) {
            ...
            case 1:
                ...
                if ($code == 'tns_hpf' && class_exists(\OnTap\MasterCard\Gateway\Config\Hpf\Config::class)) {
                    $tnsConfig = $this->simiObjectManager->get('OnTap\MasterCard\Gateway\Config\Hpf\Config');
                    $tnsConfig->setMethodCode('tns_hpf');
                    // $tnsConfig = $this->simiObjectManager->get('\Magento\Payment\Gateway\Config');
                    $storeId = $this->storeManager->getStore()->getId();
                    $detail['merchant_id']    = $tnsConfig->getMerchantId($storeId);
                    $detail['api_url']        = $tnsConfig->getApiUrl($storeId);
                    $detail['component_url']  = $tnsConfig->getComponentUrl($storeId);
                    $detail['vault_ccv']      = $tnsConfig->getValue('vault_ccv', $storeId);
                    $detail['three_d_secure'] = $tnsConfig->getValue('three_d_secure', $storeId);
                }
